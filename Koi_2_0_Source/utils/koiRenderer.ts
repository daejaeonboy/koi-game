...
export class KoiRenderer {
    ...
    public update(koi: KoiType, dt: number, isAbsolutePosition: boolean = false) {
        ...
        // 몸통이 과도하게 꺾이지 않도록 세그먼트 간 최대 굽힘 각도를 제한합니다. (라디안, 약 30도)
        ...
        for (let i = 1; i < this.segmentCount; i++) {
            ...
            // 1번 세그먼트는 머리 각도에 맞춰 정렬해, 초기 회전 시 목이 꺾이는 현상을 완화합니다.
            let targetAngle: number;
            if (i === 1) {
                targetAngle = head.angle + Math.PI;
            } else {
                const dx = cur.x - prev.x;
                const dy = cur.y - prev.y;
                const desiredAngle = Math.atan2(dy, dx);

                let diff = desiredAngle - prev.angle;
                while (diff < -Math.PI) diff += Math.PI * 2;
                while (diff > Math.PI) diff -= Math.PI * 2;

                if (diff > maxBend) diff = maxBend;
                if (diff < -maxBend) diff = -maxBend;
    ...
    // Encyclopedia-style initialization: perfectly straight horizontal pose
    public initSegmentsStraight(centerX: number, centerY: number, scale: number = 1.0) {
        this.segments = [];
        this.scale = scale;
        this.targetScale = scale;
        this.startScale = scale;
        this.spacing = this.baseSpacing * scale;

        // Calculate total body length
        const totalLength = (this.segmentCount - 1) * this.spacing;

        // 중심(centerX)을 기준으로 몸통을 배치하되, 머리(0번)는 진행 방향(+X) 쪽에 둡니다.
        const headX = centerX + totalLength / 2;

        for (let i = 0; i < this.segmentCount; i++) {
            this.segments.push({
                x: headX - i * this.spacing,
                y: centerY,
                angle: i === 0 ? 0 : Math.PI // 머리는 전방(0), 몸통은 후방(π)
            });
        }
        this.initialized = true;
    }

    public drawCentered(ctx: CanvasRenderingContext2D, width: number, height: number, colors: KoiColors, spots: Array<{ x: number, y: number, size: number, color: string }>, drawShadow: boolean = false) {
        if (!this.initialized) return;

        // Use the middle segment for centering
        const centerIndex = Math.floor(this.segmentCount / 2);
        const centerSeg = this.segments[centerIndex];

        const toLocal = (x: number, y: number) => {
            return {
                x: x - centerSeg.x + width / 2,
                y: y - centerSeg.y + height / 2
            };
        };

        // Draw shadow first if requested
        if (drawShadow) {
            this.drawShadow(ctx, { x: 5, y: 15 }, toLocal);
        }

        this.renderKoi(ctx, colors, spots, toLocal, 0, undefined, true);
    }
    ...
    public drawWorld(ctx: CanvasRenderingContext2D, colors: KoiColors, spots: Array<{ x: number, y: number, size: number, color: string }>, isSelected: boolean = false, time: number = 0, spotPhenotype?: SpotPhenotype) {
        if (!this.initialized) return;
        ...
        this.renderKoi(ctx, colors, spots, toWorld, time, spotPhenotype);
    }
    ...
    private renderKoi(ctx: CanvasRenderingContext2D, colors: KoiColors, spots: Array<{ x: number, y: number, size: number, color: string }>, transform: (x: number, y: number) => { x: number, y: number }, time: number = 0, spotPhenotype?: SpotPhenotype, isStaticPose: boolean = false) {
        ...
        // 3. Eyes (Layer 2.5)
        this.drawEyes(ctx, this.segments[0], transform, isStaticPose);
        ...
        if (spots && spots.length > 0) {
            ...
            // Apply SpotPhenotype effects if provided
            const phenotypeOpacity = spotPhenotype?.opacityBase ?? 1.0;
            const phenotypeSizeMultiplier = spotPhenotype ? (0.5 + spotPhenotype.sizeBase * 1.5) : 1.0; // 0.5x ~ 2.0x
            ...
            spots.forEach(spot => {
                ...
                if (segmentIndex >= 0 && segmentIndex < this.segmentCount) {
                    ...
                    // Apply SpotPhenotype size multiplier
                    const rawSpotRadius = (spot.size / 100) * radius * phenotypeSizeMultiplier;
                    const spotRadius = rawSpotRadius;

                    // Apply SpotPhenotype opacity and blur
                    ctx.globalAlpha = phenotypeOpacity;
                    if (phenotypeBlur > 0.1) {
                        ctx.shadowBlur = phenotypeBlur * 10; // Scale blur to pixels
    ...
    private drawEyes(ctx: CanvasRenderingContext2D, head: Segment, toLocal: (x: number, y: number) => { x: number, y: number }, isStaticPose: boolean = false) {
        ...
        // For static pose (detail modal), use full circles with body angle
        // For swimming pose (pond), use half-ellipses with eye angles
        if (isStaticPose) {
            // Static pose: Draw full ellipses aligned with body
            ctx.fillStyle = '#ffffff';
            ctx.beginPath();
            ctx.ellipse(pLeft.x, pLeft.y, (eyeSize + borderSize) * flattenX, eyeSize + borderSize, angle, 0, Math.PI * 2);
            ctx.fill();
            ctx.beginPath();
            ctx.ellipse(pRight.x, pRight.y, (eyeSize + borderSize) * flattenX, eyeSize + borderSize, angle, 0, Math.PI * 2);
            ctx.fill();

            ctx.fillStyle = '#1a1a1a';
            ctx.beginPath();
            ctx.ellipse(pLeft.x, pLeft.y, eyeSize * flattenX, eyeSize, angle, 0, Math.PI * 2);
            ctx.fill();
            ctx.beginPath();
            ctx.ellipse(pRight.x, pRight.y, eyeSize * flattenX, eyeSize, angle, 0, Math.PI * 2);
            ctx.fill();
        } else {
            // Dynamic pose: Draw half-ellipses with tilted angles
            ctx.fillStyle = '#ffffff';
            ctx.beginPath();
