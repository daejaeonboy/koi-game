...
import { createDefaultSpotPhenotypeGenes } from './genetics';

// Dictionaries for mapping Enums to Integers
// Note: Order must be preserved for backward compatibility with existing save data
const GENE_MAP = [
    GeneType.BLACK, GeneType.RED, GeneType.YELLOW,
    GeneType.WHITE, GeneType.ORANGE, GeneType.CREAM
];

const SHAPE_MAP = [
    SpotShape.CIRCLE, SpotShape.HEXAGON, SpotShape.POLYGON, SpotShape.OVAL_H, SpotShape.OVAL_V
];
    ...
    try {
        ...
        pondsArray.forEach(p => {
            ...
            const kois: Koi[] = minKois.map(mk => {
                ...
                return {
                    ...
                    genetics: {
                        ...
                        spotPhenotypeGenes: createDefaultSpotPhenotypeGenes(), // Migration: Add default genes for old data
                    },
                    position: { x: kx, y: ky },
                    velocity: { vx: 0, vy: 0 },
                    age: kAge,
            ...
            ponds[id] = {
                ...
                waterQuality: 100, // Default value for old data
            };
        });
